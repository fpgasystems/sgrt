...
merge_emconfig_json() {
        ...
        "Boards": [
            {
                "Devices": [
                    {
                        "Name": "xilinx_u55c_gen3x16_xdma_3_202210_1",
                        "DdrBanks": "",
                        "PlatformData": {
                            "plp": {
                                "slaveBridge": "enabled",
                    ...
                    {
                        "Name": "xilinx_com_xd_vck5000_1_0",
                        "DdrBanks": ""
                    }
                ],
                "NumBoards": "2"
    ...
    # Write merged_json to a temporary file
    #temp_file=$(mktemp)
    ...
    ## Initialize variables to store devices and num_boards
    #devices=""
    #num_boards=0

    ## Loop through each JSON file and merge the device objects
    #for file_path in "${json_files_array[@]}"; do
    #    devices+="$(grep -o '"Devices": \[[^]]*\]' "$file_path"),"
    #    num_boards=$((num_boards + $(grep -o '"Name"' "$file_path" | wc -l)))
    #done

    # Remove trailing comma from devices
    #devices="${devices%,}"

    # Update the "Boards" field with devices and "NumBoards" field
    #merged_json="${merged_json/\"Boards\": \[\]/\"Boards\": [{\"Devices\": [$devices], \"NumBoards\": $num_boards}]}"

    # Output the merged JSON to the specified output file
    #echo "$merged_json" > "$output_file"
}
...
case "$target_name" in
    sw_emu|hw_emu)
        ...
        for ((i = 0; i < ${#device_indexes[@]}; i++)); do
            ...
            json_files+="./_x.$xclbin_name_i.$target_name.$platform_name_i/emconfig.json "
